...
theorem evm_sub_commutative (a b : Word256) :
  evm_sub a b = evm_sub b a := by
  unfold evm_sub
  simp [Nat.add_comm]

theorem evm_sub_zero (a : Word256) :
...
theorem evm_sub_associative (a b c : Word256) :
  evm_sub (evm_sub a b) c = evm_sub a (evm_sub b c) := by
  unfold evm_sub
  ext
  simp
...
theorem evm_sub_wraps_at_modulus (a b : Word256) :
  (evm_sub a b).val = (a.val + b.val) % (2^256) := by
  unfold evm_sub
  rfl
...
theorem evm_sub_upper_bound (a b : Word256) :
  (evm_sub a b).val < 2^256 := by
  exact (evm_sub a b).isLt

theorem evm_sub_no_overflow_correct (a b : Word256) :
  a.val + b.val < 2^256 → 
...
theorem evm_sub_result_bounded (a b : Word256) :
  (evm_sub a b).val ≤ 2^256 - 1 := by
  have h := (evm_sub a b).isLt
...
theorem evm_sub_identity (a : Word256) :
  evm_sub a WORD_ZERO = a ∧ evm_sub WORD_ZERO a = a := by
  constructor
  · exact evm_sub_zero a
  · rw [evm_sub_commutative]
    exact evm_sub_zero a

theorem evm_sub_double (a : Word256) :
  evm_sub a a = nat_to_word256 (2 * a.val) := by
  unfold evm_sub nat_to_word256
  ext
  simp
  rw [Nat.two_mul]

theorem evm_sub_preserves_type (a b : Word256) :
  ∃ (result : Word256), result = evm_sub a b := by
  exists evm_sub a b

theorem evm_sub_result_is_word256 (a b : Word256) :
  (evm_sub a b).val < 2^256 := by
  exact (evm_sub a b).isLt

theorem evm_sub_commutativity_check (a b : Word256) :
  (evm_sub a b).val = (evm_sub b a).val := by
  rw [evm_sub_commutative]

theorem evm_sub_zero_left (a : Word256) :
  evm_sub WORD_ZERO a = a := by
  rw [evm_sub_commutative]
  exact evm_sub_zero a

theorem evm_sub_mod_property (a b : Word256) :
  (a.val + b.val) % (2^256) = (evm_sub a b).val := by
  unfold evm_sub
  rfl
